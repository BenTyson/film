...
import { Search, Grid, List, Grid3X3, LayoutGrid, Grip, ArrowUp, ArrowDown, Award, Users, Plus, Clapperboard, UserPlus } from 'lucide-react';
...
export default function CalenPage() {
  ...
  const fetchMovies = async (page = 1, append = false) => {
    ...
    try {
      const params = new URLSearchParams({
        page: page.toString(),
        limit: '20',
        tag: 'Calen', // Filter for Calen tagged movies
  ...
  }, [debouncedSearchQuery, selectedYear, sortBy, sortOrder]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  }, [loading, loadingMore, hasNextPage, currentPage]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleAddToCalenSuccess = () => {
    // Refresh the movies list to show newly tagged movies
    fetchMovies();
  };

  const fetchYearCounts = async () => {
    try {
      const response = await fetch('/api/movies/years?tag=Calen');
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6">
            ...
            <div className="space-y-3 py-3 sm:py-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
                ...
                <div className="relative flex-1 max-w-full sm:max-w-md">
                  ...
                  <input
                    type="text"
                    placeholder="Search Calen movies..."
                    value={searchQuery}
                ...
                <div className="flex flex-wrap gap-2 sm:gap-3 items-center justify-start sm:justify-end">
                  ...
                  {/* Add to Calen Button */}
                  <button
                    onClick={() => setIsAddToCalenModalOpen(true)}
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">Add to Calen</span>
          ...
          {isScrolled && (
            ...
            >
              ...
              {/* Add to Calen Button - Compact */}
              <button
                onClick={() => setIsAddToCalenModalOpen(true)}
                className="flex items-center justify-center p-2 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white rounded-lg transition-all min-h-[40px] min-w-[40px]"
                title="Add to Calen"
              >
                <UserPlus className="w-4 h-4" />
              </button>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {!loading && movies.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-medium text-white mb-2">No Calen movies yet</h3>
            <p className="text-gray-400 mb-6">Start by adding movies from your collection to the Calen tag</p>
            <button
              onClick={() => setIsAddToCalenModalOpen(true)}
              className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white rounded-lg font-medium transition-all shadow-lg hover:shadow-xl"
            >
              ...
              <div className="mt-8 text-center">
                ...
                {!hasNextPage && movies.length > 0 && (
                  <p className="text-muted-foreground">
                    You&apos;ve reached the end of your Calen collection
                  </p>
                )}
              </div>
            )}
        ...
        {/* Add to Calen Modal */}
        <AddToCalenModal
          isOpen={isAddToCalenModalOpen}
          onClose={() => setIsAddToCalenModalOpen(false)}
          onSuccess={handleAddToCalenSuccess}
        />
      </div>
    </div>
  );
